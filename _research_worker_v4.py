#!/usr/bin/env python3
"""Borg Research Worker v2.4 - Reliable enrichment with multi-format parsing"""
...
def parse_llm_response(raw):
    """Parse LLM response - handles JSON, code blocks, markdown, and plain text."""
    if not raw:
        return None
    ...
    # 2. JSON in code blocks - try harder to extract
    for delim in ["```json", "```"]:
        if delim in text:
            try:
                ...
                for part in parts[1:]:
                    ...
                    try:
                        ...
                        pass
                    # Fix broken JSON with mixed quote styles (e.g., "key": "value", "key2": "value2")
                    # where the LLM outputs: "KEY1": "val1", "KEY2": "val2",
                    # which is not valid JSON - needs to be wrapped in {}
                    if '"CATEGORY"' in fixed or '"SHORT_DESCRIPTION"' in fixed:
                        wrapped = '{' + fixed + '}'
    ...
    # 4. Plain-text / markdown key-value format
    result = {}

    # Normalize: remove backslash-escaping of underscores (SHORT\_DESCRIPTION -> SHORT_DESCRIPTION)
    norm_text = text.replace('\\_', '_')

    # Strip preamble (e.g., "Here is a classification of...")
    stripped = re.sub(
    ...
    # Patterns for various markdown formats:
    # **CATEGORY:** value, **SHORT_DESCRIPTION:** value (multi-line possible)
    # - CATEGORY: value
    # CATEGORY: value
    # Also handle bullet lists for FEATURES/TAGS
    kv_patterns = {
    ...
    for key, pat in kv_patterns.items():
        ...
        if m:
            val = m.group(1).strip()
            # Clean up: remove leading * and whitespace, strip markdown
            val = re.sub(r'^[\s:*-]+', '', val).strip()
            # Collapse multi-line bullet lists into comma-separated
            val = re.sub(r'\n[-*]\s+', ', ', val)
            val = re.sub(r'\n+', ' ', val)
    ...
    # Also try to extract classification from narrative responses
    if 'CATEGORY' not in result:
        # Look for category-like patterns in the narrative
        # Check **Resource Classification:** **value** pattern
        cls_match = re.search(r'\*\*Resource\s+Classification:\*\*\s*\*\*([^*]+)\*\*', stripped, re.IGNORECASE)
        if cls_match:
            result['CATEGORY'] = cls_match.group(1).strip()
        else:
            # Try to match against known taxonomy
...
def main():
    ...
    logger.info(f"Borg Research Worker v2.4 starting")
    ...
    for idx, (eid, url, sd, is_gh, innov) in enumerate(all_entries):
        ...
        if not rdata:
            ...
            with open(dump_path, 'a', encoding='utf-8') as df:
                df.write(json.dumps({"url": url, "model": model, "raw": raw[:1000]}) + '\n')
            logger.warning(f"  Parse failed (model={model}): {raw[:100]}")
            failed += 1
            continue
