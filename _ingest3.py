...
for (u,) in a.fetchall():
    nu = (
        u.lower().strip().rstrip("/").replace("http://", "https://").replace("www.", "")
    )
    if "#" in nu:
        nu = nu.split("#")[0]
    if nu.endswith("/"):
...
SUBCAT_MAP = {
    ...
    "Context Engineering & Isolation": {
        "RAG Pipelines": ["rag", "retrieval", "document", "chunking"],
    ...
    "Memory & Persistence Architecture": {
        ...
        "Storage Engines": ["storage", "persist", "cache", "surrealdb"],
        "MCP Servers": ["mcp server", "mcp-server"],
    },
    "Interface & Developer UX": {
        "Chat Interfaces": ["chat", "assistant"],
        "Browser Automation": ["browser", "puppeteer", "playwright"],
        "Terminal & CLI": ["terminal", "cli", "command", "tui"],
        "MCP Servers": ["mcp server", "mcp-server"],
    },
    "Connectivity / MCP / A2A": {
    ...
    "Coding Harness Tools": {
        "Spec-Driven Development": ["spec", "harness", "governance", "bmad"],
...
for url in unique:
    ...
    signal = min(
        100,
        max(0, int(round((innovation * 4) + (quality * 30) + desc_score + gh_bonus))),
    )
    ...
    a.execute(
        """INSERT OR IGNORE INTO entries (id, url, page_title, short_description, long_description,
