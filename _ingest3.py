...
HARD_REJECT_SUBS = {
    ...
    'sports','relationship_advice','aihub','psychologytalk',
    'aging','casualconversation','nofilternews','48lawsofpower',
    'futurology','erotichypnosis','musicindustry','inventors',
    'askteddit','askmanagers','movementdemf',
}
...
TOOL_SUBS = {
    ...
    'claudeplayspokemon','ai_trading','trading','opencodecli',
    'knowledgegraph','rag','ollama','aimemory','aigentsinaction',
    'opensourceai','bard','webafterai','google_antigravity',
    'langchain','surrealdb','notebooklm','trae_ai','bmad_method',
    'ai_application','swebench','googlegemini','artificialntelligence',
}
...
TOOL_INDICATORS = [
    ...
    'harness','codex','gemini','opencode','cli','tui',
    'coding agent','benchmark','memory layer','graph',
    '12-factor','skill','control plane','armorer',
    'surrealdb','notebook','trae','bmad','rag','embedding',
    'vector','chunking','fragmentation','retention',
]

CATEGORY_KEYWORDS = {
    'Agent Orchestration & Workflow': ['agent','orchestrat','workflow','langgraph','crewai','autogen','agentic','multi-agent','pipeline','hermes','12-factor','armorer','shadowbroker'],
    'Context Engineering & Isolation': ['rag','context','embed','chunk','retrieval','prompt','template','isolat','contextium','chunking'],
    'Memory & Persistence Architecture': ['memory','persist','graph','knowledge','recall','mem0','memgpt','cache','glia','surrealdb','fragmentation','sqlitevec'],
    'Interface & Developer UX': ['browser','desktop','gui','chat','interface','dashboard','ux','voice','terminal','cli','tui'],
    ...
    'Guides & Industry Trends': ['guide','tutorial','awesome','benchmark','trend','report','survey','analysis','strategy','lesswrong','ycombinator','wsj','karpathy'],
    'Coding Harness Tools': ['harness','codex','claude code','skill','hook','governance','task-master','taskmaster','coding agent','opencode','bmad'],
    'AI Agents & Frameworks': ['ai agent','framework','autonomous','manus','devin','swe-agent','research agent','browser agent','sana'],
    'Search & Discovery': ['search','tavily','serp','brave search','code search','scrape','crawl','fetch','discovery'],
    'Coding Tools & IDEs': ['copilot','autocomplete','ide','editor','kiro','cursor','cline','jetbrains','vscode','gemini cli','vibecoding','trae'],
    'Developer Workflow & Tools': ['git','github','ci/cd','pipeline','deploy','documentation','testing','monitor','devops','sysadmin'],
    'Vector Databases & Embeddings': ['vector','embedding','pgvector','chromadb','pinecone','qdrant','weaviate','sqlitevec'],
    'Security & Red Teaming': ['security','vulnerab','guardrail','firewall','audit','scan','pentest','threat','red team','hack','netsec'],
}

SUBCAT_MAP = {
    'Agent Orchestration & Workflow': {'Agentic Frameworks':['agent','framework','orchestrat','agentic','multi-agent','hermes','12-factor'],'Workflow & Pipeline Engines':['workflow','pipeline','n8n','temporal'],'Tool Discovery & Routing':['tool','mcp','bridge','gateway','routing'],'MCP Servers':['mcp server','mcp-server']},
    'Context Engineering & Isolation': {'RAG Pipelines':['rag','retrieval','document','chunking'],'Prompt Engineering':['prompt','template'],'Embedding & Indexing':['embed','index','semantic'],'MCP Servers':['mcp server','mcp-server']},
    'Memory & Persistence Architecture': {'Memory Systems':['memory','mem0','memgpt','recall','knowledge graph','glia','sqlitevec'],'Storage Engines':['storage','persist','cache','surrealdb'],'MCP Servers':['mcp server','mcp-server']},
    'Interface & Developer UX': {'Chat Interfaces':['chat','assistant'],'Browser Automation':['browser','puppeteer','playwright'],'Terminal & CLI':['terminal','cli','command','tui'],'MCP Servers':['mcp server','mcp-server']},
    ...
    'Guides & Industry Trends': {'Tutorials & Courses':['tutorial','course','guide','learn'],'Industry Analysis':['trend','report','survey','landscape','wsj','karpathy'],'Curated Lists':['awesome','curated','collection']},
    'Coding Harness Tools': {'Spec-Driven Development':['spec','harness','governance','bmad'],'Coding Agents':['coding agent','codex','claude code'],'Task Management':['task','task-master','taskmaster'],'MCP Servers':['mcp server','mcp-server']},
    ...
    'Coding Tools & IDEs': {'AI Code Completion':['copilot','autocomplete','suggest'],'IDE Extensions':['vscode','extension','jetbrains','kiro','cursor','cline','trae'],'AI Editors & IDEs':['ide','editor','gemini cli','vibe']},
    ...
    'Vector Databases & Embeddings': {'Vector Databases':['vector','pgvector','chromadb','pinecone','qdrant','sqlitevec'],'Embedding Models':['embed','sentence-transformer']},
...
for url in unique:
    ...
    # Hard reject: bare reddit.com
    if re.search(r'^https?://(www\.)?reddit\.com/?$', url, re.IGNORECASE):
        skipped_noise += 1
        continue
    ...
    a.execute("SELECT COUNT(*) FROM layer_membership WHERE entry_id=? AND layer=?", (eid, layer))
    if a.fetchone()[0] == 0:
