#!/usr/bin/env python3
"""
Phase 4: Rebuild links_backlog from atlas.db entries
"""
...
def extract_links_backlog():
    ...
    # Check if links_backlog exists in main DB
    conn = sqlite3.connect(str(DB_PATH))
    cur = conn.cursor()

    # Find links_backlog table
    cur.execute(
    ...
    print(f"links_backlog columns: {cols}")

    # Get existing backlog count
    ...
    atlas_cur.execute("SELECT COUNT(*) FROM entries")
    total = atlas_cur.fetchone()[0]
    print(f"ATLAS total entries: {total}")

    # Fetch all URLs with their data
    atlas_cur.execute(
        ...
        if len(batch) >= 100:
            try:
                conn.executemany(
                    ...
                    (uuid, url, normalized_url, title, description, tags, source, is_duplicate, duplicate_of, research_status,
                     http_status, page_title, page_description, discovered_at, last_attempted_at, completed_at, error_message,
                     metadata, favicon_url, researched_at, cluster_id, bobbybookmarks_bookmark_id, import_session_id,
    ...
    if batch:
        try:
            conn.executemany(
                """
                INSERT OR IGNORE INTO links_backlog 
                (uuid, url, title, description, source, status, metadata, discovered_at, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                batch,
            )
    ...
    print(f"\nLinks backlog: {existing} → {total_after} (+{total_after - existing})")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Phase 4: Rebuilding links_backlog from ATLAS")
    print("=" * 60)
    count = extract_links_backlog()
    print(f"\nPhase 4 complete: {count} links inserted")
